...
module.exports = (req, res, next) => {
    try {
        ...
        try {
            ...
            //Actualiza token
            return RenovationTokens.findOneAndUpdate({ userId: decodedToken.sub }, { $inc: { updatesNumber: 1, requests: 1 } })
                .then(renovationToken => {
                    if (renovationToken) {
                        const currentDate = new Date()
                        if (renovationToken.updatesNumber < REQUESTS_LIMIT_MINUTES) {
                            if (currentDate - renovationToken.renovationDate >= 60000) {
                                //Incrementa updates
                                return RenovationTokens.findOneAndUpdate({ userId: decodedToken.sub }, { updatesNumber: 0 })
                            }
                        } else {
                            return RenovationTokens.findByIdAndDelete(renovationToken._id)
                                .then(() => {
                                    return Users.findByIdAndUpdate({ _id: decodedToken.sub }, { isBanned: true, banDate: currentDate })
                                    .then(() => {
                                        res.status(403).json({ error: 'request limit' })
                                    })
                                })
                        }
                        next()
